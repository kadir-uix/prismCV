...
const RainbowMaterial = shaderMaterial(
  ...
  /*glsl*/ ` varying vec2 vUv;
    ...
    vec3 physhue2rgb(float hue, float ratio) {
      return smoothstep(vec3(0.0),vec3(1.0), abs(mod(hue + vec3(0.0,1.0,0.0)*ratio,2.0)*2.0-1.0));
    }
    ...
    vec3 iridescence (float angle, float thickness) {      
      ...
      float lum = 0.0064;      
      float luma = 0.01070;      
      vec3 tint = vec3(0.639,0.78252,0.8723);      
      float interf0 = 2.0;      
      float phase0 = 1.0 / 2.8;      
      float interf1 = interf0 * 2.0 / 3.0;      
      float phase1 = phase0;      
      float f = (2.0 - NxV) * (1.0 - NxV);
      ...
      vec3 film = hue * lum + vec3(0.5,0.5,0.6) * luma;      
    ...
    vec3 spectral_zucconi6(float w, float t) {    
      ...
      const vec3 c1 = vec3(3.94585104, 2.93225262, 2.41593945);
    ...
    void main() {
      ...
      vec3 co = c / iridescence(uv.x * 0.5 * 3.14159, 1.0 - uv.y + time / .0) / 20.0;      
